...
#[unstable(
    feature = "twizzler_kernel_console_read",
    issue = "none",
    reason = "not all errors have been implemented"
)]
impl From<KernelConsoleReadError> for io::Error {
    fn from(x: KernelConsoleReadError) -> Self {
        match x {
            KernelConsoleReadError::Unknown => Self::from(io::ErrorKind::Other),
...
impl io::Read for Stdin {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        twizzler_abi::syscall::sys_kernel_console_read(
            buf,
